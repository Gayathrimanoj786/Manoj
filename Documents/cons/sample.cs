...
public class Login
{
    ...
    public void load()
    {
        ...
        while (c < ss1)
        {
            ...
            pass.Add(sr.ReadLine());
            type.Add(Convert.ToInt32(sr.ReadLine()));
    ...
    public void display()
    {
        for (int i = 0; i < user.Count; i++)
            Console.WriteLine(user[i] + " " + pass[i] + "  " + type[i]);
    }
    public int authenticate(string user1, string pass1)
    {
                 ...
                 if (user[i].Equals(user1))
            {
                ...
                if (pass[i].Equals(pass1))
                {
                    return Convert.ToInt32(type[i]);
...
abstract public class Product
{
    ...
    public void initialize()
    {
        try
        {
            ...
            StreamReader sr = new StreamReader("D://NKK/GAYATHRI/item.txt");
            sr.BaseStream.Seek(0, SeekOrigin.Begin);
            int s = Convert.ToInt16(sr.ReadLine());
            ...
            while (c < s)
            {
                ...
                product_name.Add(sr.ReadLine());
                cost.Add(Convert.ToDouble(sr.ReadLine()));
                quantity.Add(Convert.ToInt32(sr.ReadLine()));
    ...
    public void add()
    {
        ...
        {
            Console.WriteLine("Input Format is Wrong" + e);
        }
...
public interface Iverification_card
{
     int verification(string user,string ccv1);
}
public class Card_Payment : Cart_Product, Iverification_card
{
...
sealed class Payment : Card_Payment
{
    ...
    public ArrayList time_info = new ArrayList();
            ...
            if (cart_product_name.Count > 0)
            {
                if (verification(user, cvv) == 1)
                {
                    ...
                    for (int i = 0; i < cart_product_name.Count; i++)
                    {
                        ...
                        time_info.Add(DateTime.Now.ToString());
                    }
                    display_pay();
    ...
    public void display_pay()
    {
        for (int i = 0; i < pay_product_name.Count; i++)
            Console.WriteLine(pay_id[i] + "  " + pay_product_name[i] + "  " + pay_quantity[i] + "  " + pay_total_cost[i] + " " + time_info[i]);
